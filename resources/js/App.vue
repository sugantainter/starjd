    ...
    >
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <span class="hidden text-lg font-bold tracking-tight text-[#1a1a1a] sm:text-xl">StarJD</span>
                </router-link>
                    ...
                    <template v-if="navUser">
                        <div class="relative" ref="navUserMenuRef">
                            <button
                                ...
                                :class="navUser.role === 'creator' ? 'hover:border-[#10b981] focus:ring-[#10b981]/20' : navUser.role === 'admin' ? 'hover:border-[#1e293b] focus:ring-[#1e293b]/20' : ''"
                                @click="navUserMenuOpen = !navUserMenuOpen"
                            >
                                <span class="max-w-[120px] truncate">{{ navUser.name }}</span>
                                <svg class="h-4 w-4 shrink-0 text-[#64748b] transition-transform" :class="{ 'rotate-180': navUserMenuOpen }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
                            </button>
                            <Transition
                                enter-active-class="transition duration-100 ease-out"
                                enter-from-class="opacity-0 scale-95"
                            ...
                            >
                                ...
                                >
                                    <div class="border-b border-[#e2e8f0] px-4 py-2.5">
                                        <p class="text-xs font-medium uppercase tracking-wide text-[#64748b]">{{ navUser.role }}</p>
                                        <p class="truncate text-sm font-medium text-[#1a1a1a]">{{ navUser.name }}</p>
                                    </div>
                                    <template v-if="navUser.role === 'creator'">
                                        <router-link to="/creator/dashboard" class="block px-4 py-2.5 text-sm text-[#1a1a1a] transition hover:bg-[#10b981]/5 hover:text-[#10b981]" @click="navUserMenuOpen = false">Creator Dashboard</router-link>
                                        <router-link to="/creator/profile" class="block px-4 py-2.5 text-sm text-[#1a1a1a] transition hover:bg-[#10b981]/5 hover:text-[#10b981]" @click="navUserMenuOpen = false">My Profile</router-link>
                                    </template>
                                    <template v-else-if="navUser.role === 'brand'">
                                        <router-link to="/brand/dashboard" class="block px-4 py-2.5 text-sm text-[#1a1a1a] transition hover:bg-[#e63946]/5 hover:text-[#e63946]" @click="navUserMenuOpen = false">Brand Dashboard</router-link>
                                        <router-link to="/brand/creators" class="block px-4 py-2.5 text-sm text-[#1a1a1a] transition hover:bg-[#e63946]/5 hover:text-[#e63946]" @click="navUserMenuOpen = false">Discover Creators</router-link>
                                    </template>
                                    <template v-else-if="navUser.role === 'admin'">
                                        <router-link to="/admin" class="block px-4 py-2.5 text-sm text-[#1a1a1a] transition hover:bg-[#1e293b]/5 hover:text-[#1e293b]" @click="navUserMenuOpen = false">Admin Panel</router-link>
                                    </template>
                                    <div class="border-t border-[#e2e8f0] pt-1">
                                        <button type="button" class="block w-full px-4 py-2.5 text-left text-sm text-[#64748b] transition hover:bg-red-50 hover:text-red-600" @click="navLogout">Logout</button>
                                    </div>
                                </div>
                            </Transition>
                        </div>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <template v-if="navUser">
                                <div class="my-2 border-t border-[#e5e7eb] pt-2">
                                    <p class="px-4 py-1 text-xs font-medium uppercase text-[#64748b]">{{ navUser.role }}</p>
                                    <p class="truncate px-4 py-1 text-sm font-medium text-[#1a1a1a]">{{ navUser.name }}</p>
                                </div>
                                <template v-if="navUser.role === 'creator'">
                                ...
                                <button type="button" class="cursor-link rounded-lg px-4 py-3 text-left text-sm text-red-600 transition hover:bg-red-50" @click="navLogout(); navMobileOpen = false">Logout</button>
                            </template>
                            <template v-else>
        ...
        >
            ...
            <div class="hero-wallpaper absolute inset-0" aria-hidden="true">
                <div class="hero-wallpaper-scroll">
                    <div
                        ...
                        :style="{ backgroundImage: `url(${typeof img === 'object' && img?.src ? img.src : img})` }"
                    />
                </div>
            </div>
            ...
            >
                ...
                <div class="hero-content relative w-full max-w-4xl mx-auto px-0 sm:px-2">
                    <h1
                    ...
                    >
                        <span class="hero-row hero-row-1 block text-[#1a1a1a]">{{ heroData?.headline || 'Influencer Marketing Made Easy' }}</span>
                    </h1>
                    <div class="animate-fade-in-up animation-delay-300 mt-4 flex w-full min-w-0 flex-wrap justify-center gap-2 px-0 sm:mt-5 sm:gap-3 sm:px-2">
                        <router-link v-if="isInternalHeroUrl(heroData?.btn_creator_url)" :to="heroData.btn_creator_url || '/creator-landing'" class="cursor-link btn-hero-creator shrink-0 rounded-xl border border-emerald-400/60 bg-white/95 px-3 py-2 text-sm font-medium text-[#059669] shadow-lg backdrop-blur-sm transition hover:border-emerald-500 hover:bg-emerald-50 hover:shadow-xl sm:px-5 sm:py-2.5">{{ heroData?.btn_creator_label || 'Join as Creator' }}</router-link>
                        <a v-else :href="heroData?.btn_creator_url || '#join-creator'" class="cursor-link btn-hero-creator shrink-0 rounded-xl border border-emerald-400/60 bg-white/95 px-3 py-2 text-sm font-medium text-[#059669] shadow-lg backdrop-blur-sm transition hover:border-emerald-500 hover:bg-emerald-50 hover:shadow-xl sm:px-5 sm:py-2.5">{{ heroData?.btn_creator_label || 'Join as Creator' }}</a>
                        <router-link v-if="isInternalHeroUrl(heroData?.btn_brand_url)" :to="heroData.btn_brand_url || '/brand-landing'" class="cursor-link join-brand shrink-0 rounded-xl bg-[#e63946] px-3 py-2 text-sm font-medium text-white shadow-lg shadow-[#e63946]/35 transition hover:bg-[#c1121f] hover:shadow-xl hover:shadow-[#e63946]/45 sm:px-5 sm:py-2.5">{{ heroData?.btn_brand_label || 'Join as Brand' }}</router-link>
                        <a v-else :href="heroData?.btn_brand_url || '#join-brand'" class="cursor-link join-brand shrink-0 rounded-xl bg-[#e63946] px-3 py-2 text-sm font-medium text-white shadow-lg shadow-[#e63946]/35 transition hover:bg-[#c1121f] hover:shadow-xl hover:shadow-[#e63946]/45 sm:px-5 sm:py-2.5">{{ heroData?.btn_brand_label || 'Join as Brand' }}</a>
                        <router-link v-if="isInternalHeroUrl(heroData?.btn_browse_url)" :to="heroData.btn_browse_url || '/creators'" class="cursor-link btn-hero-browse shrink-0 rounded-xl border border-[#e5e7eb] bg-white/95 px-3 py-2 text-sm font-medium text-[#374151] shadow-lg backdrop-blur-sm transition hover:border-pink-300 hover:bg-pink-50 hover:shadow-xl sm:px-5 sm:py-2.5">{{ heroData?.btn_browse_label || 'Browse Creators' }}</router-link>
                        <a v-else :href="heroData?.btn_browse_url || '#featured'" class="cursor-link btn-hero-browse shrink-0 rounded-xl border border-[#e5e7eb] bg-white/95 px-3 py-2 text-sm font-medium text-[#374151] shadow-lg backdrop-blur-sm transition hover:border-pink-300 hover:bg-pink-50 hover:shadow-xl sm:px-5 sm:py-2.5">{{ heroData?.btn_browse_label || 'Browse Creators' }}</a>
                    </div>
                </div>
                ...
                >
                    ...
                    >
                        <component
                            :is="cascadeHasLink(img) ? (isInternalHeroUrl(img.link) ? 'router-link' : 'a') : 'div'"
                            v-for="(img, i) in galleryCascadeImagesComputed"
                            :key="i"
                            :to="cascadeHasLink(img) && isInternalHeroUrl(img.link) ? img.link : undefined"
                            :href="cascadeHasLink(img) && !isInternalHeroUrl(img.link) ? img.link : undefined"
                            ...
                            :class="[
                                ...
                                { 'gallery-cascade-card-active': activeCascadeIndex === i },
                            ]"
        ...
        <section class="border-b border-[#e5e7eb] bg-[#fafaf9] px-3 py-8 sm:px-4 sm:py-10 md:py-14">
            <div class="mx-auto max-w-4xl">
                <h2 class="mb-4 text-center text-lg font-semibold text-[#1a1a1a] sm:mb-5 sm:text-xl md:text-2xl">Find creators</h2>

                <!-- Filters row – above search bar; single line on desktop -->
                <div class="mb-4 grid grid-cols-2 gap-3 sm:mb-5 sm:flex sm:flex-wrap sm:items-end sm:justify-center sm:gap-4 md:flex-nowrap">
                    <div class="min-w-0 sm:w-auto sm:min-w-[130px]">
                        <label for="hero-filter-category" class="mb-1 block text-xs font-medium text-[#6b7280]">Category</label>
                        <select
                        ...
                        >
                            ...
                            <option v-for="c in heroFilterOptions.categories" :key="c" :value="c">{{ c }}</option>
                        </select>
                    </div>
                    <div class="min-w-0 sm:w-auto sm:min-w-[130px]">
                        <label for="hero-filter-gender" class="mb-1 block text-xs font-medium text-[#6b7280]">Gender</label>
                        <select
                        ...
                        >
                            ...
                            <option v-for="(label, key) in heroFilterOptions.genders" :key="key" :value="key">{{ label }}</option>
                        </select>
                    </div>
                    <div class="min-w-0 sm:w-auto sm:min-w-[130px]">
                        <label for="hero-filter-language" class="mb-1 block text-xs font-medium text-[#6b7280]">Language</label>
                        <select
                        ...
                        >
                            <option value="">Any</option>
                            <option v-for="lang in heroFilterOptions.languages" :key="lang" :value="lang">{{ lang }}</option>
                        </select>
                    </div>
                    <div class="min-w-0 sm:w-auto sm:min-w-[130px]">
                        <label for="hero-filter-platform" class="mb-1 block text-xs font-medium text-[#6b7280]">Platform</label>
                        <select
                        ...
                        >
                            <option value="">Any</option>
                            <option v-for="(label, key) in heroPlatformOptions" :key="key" :value="key">{{ label }}</option>
                        </select>
                    </div>
                    <div class="min-w-0 sm:w-auto sm:min-w-[110px]">
                        <label for="hero-filter-minrate" class="mb-1 block text-xs font-medium text-[#6b7280]">Min price (₹)</label>
                        <input
                ...
                >
                    <div class="order-2 min-w-0 flex-1 sm:order-none sm:max-w-[140px]">
                        <label class="mb-0.5 block text-xs font-medium text-[#6b7280]">Platform</label>
                        <select
                            v-model="heroSearchPlatform"
                    ...
                    <div class="order-1 min-w-0 flex-1 sm:order-none">
                        <label class="mb-0.5 block text-xs font-medium text-[#6b7280]">Keywords</label>
                        <input
        ...
        <section v-if="partners.length" class="partners-marquee border-b border-[#e5e7eb] bg-white py-8">
            <div class="partners-marquee-track overflow-hidden">
                <div class="partners-marquee-inner flex items-center gap-10">
                    <template v-for="(set, setIdx) in [partners, partners]" :key="'set-' + setIdx">
                        <template v-for="p in set" :key="setIdx + '-' + p.id">
                            ...
                            >
                                <img v-if="p.logo_url" :src="p.logo_url" :alt="p.name || 'Partner'" class="h-10 max-h-12 w-auto max-w-[140px] object-contain" />
                                <span v-else class="text-lg font-semibold text-[#9ca3af]">{{ p.name || 'Partner' }}</span>
                            </a>
                            <span v-else class="partners-marquee-item flex shrink-0 items-center justify-center">
                                <img v-if="p.logo_url" :src="p.logo_url" :alt="p.name || 'Partner'" class="h-10 max-h-12 w-auto max-w-[140px] object-contain" />
                                <span v-else class="text-lg font-semibold text-[#9ca3af]">{{ p.name || 'Partner' }}</span>
                            </span>
                        </template>
            ...
            <div class="mx-auto max-w-6xl">
                <div class="grid items-center gap-12 md:grid-cols-2 md:gap-16">
                    ...
                    <div>
                        ...
                        <a
                            href="#"
                            class="cursor-link join-brand mt-8 inline-block rounded-xl bg-[#e63946] px-6 py-3 font-semibold text-white transition hover:bg-[#c1121f]"
                            >Join as Brand</a
                        >
                    </div>
                </div>
        ...
        <section id="featured" class="animate-on-scroll px-4 py-16 md:py-24">
            <div class="mx-auto max-w-6xl">
                ...
                >
                    ...
                    <router-link
                        to="/creators"
                        class="cursor-link text-sm font-semibold text-[#e63946] transition hover:underline"
                        >See All</router-link
                    >
                </div>
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    >
                        <router-link :to="'/creators/' + (creator.slug || creator.id)" class="block">
                        <div
                            class="relative aspect-[4/5] overflow-hidden bg-[#f3f4f6]"
                        >
                            <img
                                :src="creator.image || 'https://ui-avatars.com/api?name=' + encodeURIComponent(creator.name || '') + '&size=400'"
                                :alt="creator.name"
                        ...
                        <div class="p-4">
                            <div class="flex items-center justify-between gap-2">
                                <h3 class="font-semibold text-[#1a1a1a]">{{ creator.name }}</h3>
                                <span v-if="creator.rating" class="shrink-0 text-sm font-medium text-[#1a1a1a]">★ {{ creator.rating }}</span>
                            </div>
                            <p v-if="creator.tagline" class="mt-1 line-clamp-2 text-sm text-[#6b7280]">{{ creator.tagline }}</p>
                            <p v-if="creator.location" class="mt-2 text-xs text-[#6b7280]">{{ creator.location }}</p>
                            <p v-if="creator.price != null" class="mt-2 text-lg font-bold text-[#e63946]">From ₹{{ creator.price }}/project</p>
                        </div>
                        </router-link>
                    </article>
                </div>
                ...
                >
                    ...
                    <div class="videos-featured flex-1 shrink-0 lg:min-w-0">
                        ...
                        >
                            ...
                            <div class="border-t border-[#e5e7eb] bg-white p-4 md:p-5">
                                <h3 class="font-semibold text-[#1a1a1a]">{{ youtubeVideos[selectedVideoIndex].title }}</h3>
                                <p v-if="youtubeVideos[selectedVideoIndex].desc" class="mt-1 text-sm text-[#6b7280]">{{ youtubeVideos[selectedVideoIndex].desc }}</p>
                                <a
                                    :href="youtubeVideos[selectedVideoIndex].watchUrl"
                                    target="_blank"
                                    rel="noopener noreferrer"
                    ...
                    <div class="videos-sidebar w-full lg:w-[340px] lg:shrink-0">
                        <div class="rounded-2xl border border-[#e5e7eb] bg-white p-3 shadow-sm">
                            <p class="mb-3 px-2 text-xs font-semibold uppercase tracking-wider text-[#6b7280]">More videos</p>
                            <button
                                ...
                                :class="{ 'bg-[#e63946]/5 ring-1 ring-[#e63946]/30': selectedVideoIndex === i }"
                                @click="selectedVideoIndex = i"
                            >
                                <div class="relative h-16 w-28 shrink-0 overflow-hidden rounded-lg bg-[#1a1a1a]">
                                    <img :src="`https://img.youtube.com/vi/${video.videoId}/mqdefault.jpg`" :alt="video.title" class="h-full w-full object-cover" />
                                    <span class="absolute inset-0 flex items-center justify-center bg-black/40 transition group-hover:bg-black/50">
                                        <svg class="h-8 w-8 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                                    </span>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <h4 class="line-clamp-2 text-sm font-medium text-[#1a1a1a]">{{ video.title }}</h4>
                                    <span class="mt-0.5 inline-block text-xs font-medium text-[#e63946]">Watch →</span>
                                </div>
                            </button>
                            <router-link to="/videos" class="cursor-link mt-3 block rounded-xl border border-[#e5e7eb] px-4 py-2.5 text-center text-sm font-medium text-[#e63946] transition hover:bg-[#e63946]/5 hover:border-[#e63946]/50">
                                View all videos →
                            </router-link>
                        </div>
                    </div>
            ...
            <div class="mx-auto max-w-4xl">
                ...
                >
                    ...
                    >
                        ...
                        <a
                            href="#"
                            class="cursor-link join-brand inline-block rounded-xl border-2 border-white bg-transparent px-8 py-3 font-semibold text-white transition hover:bg-white hover:text-[#1a1a1a]"
                            >Join as Brand</a
                        >
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="flex flex-wrap gap-8">
                        ...
                        <div>
                            ...
                            <ul class="mt-2 space-y-2 text-sm text-[#6b7280]">
                                <li>
                                    <router-link to="/privacy" class="cursor-link transition hover:text-[#e63946]">Privacy</router-link>
                                </li>
                                <li>
                                    <router-link to="/terms" class="cursor-link transition hover:text-[#e63946]">Terms</router-link>
                                </li>
                                <li>
                                    <router-link to="/contact" class="cursor-link transition hover:text-[#e63946]">Contact</router-link>
                                </li>
                            </ul>
                        </div>
                    </div>
...
const heroFilterOptions = ref({ categories: [], genders: {}, languages: [], platforms: {} });
...
function submitHeroSearch() {
    ...
    if (heroSearchMinRate.value !== "" && heroSearchMinRate.value != null) q.min_rate = heroSearchMinRate.value;
...
const heroWallpaperScrollImagesComputed = computed(() => {
    const imgs = heroData.value?.wallpaper_images?.length ? heroData.value.wallpaper_images : heroWallpaperImages;
    return [...imgs, ...imgs];
});
const galleryCascadeImagesComputed = computed(() => {
    const imgs = heroData.value?.cascade_images?.length ? heroData.value.cascade_images : galleryCascadeImages;
    return imgs;
});
...
const steps = ref([
    { title: "Search Creators", desc: "Browse thousands of vetted creators. Filter by niche, location, and budget." },
    { title: "Chat & Book", desc: "Message creators directly and book packages. Payment is held securely until delivery." },
    { title: "Get Content", desc: "Receive high-quality content on time. Approve and download from your dashboard." },
]);
...
const youtubeVideos = ref([
    { videoId: "CaZxAzb7PPM", title: "Influencer Marketing Guide", embedUrl: "https://www.youtube.com/embed/CaZxAzb7PPM", watchUrl: "https://www.youtube.com/watch?v=CaZxAzb7PPM", desc: "Learn how to run effective influencer campaigns." },
    { videoId: "K1ImD48UqNQ", title: "Creator Success Stories", embedUrl: "https://www.youtube.com/embed/K1ImD48UqNQ", watchUrl: "https://www.youtube.com/watch?v=K1ImD48UqNQ", desc: "Hear from brands and creators who found success." },
    { videoId: "c64h-K74bFI", title: "UGC & Brand Collabs", embedUrl: "https://www.youtube.com/embed/c64h-K74bFI", watchUrl: "https://www.youtube.com/watch?v=c64h-K74bFI", desc: "How to create authentic UGC content with brands." },
    { videoId: "5qap5aO4i9A", title: "How to Find the Right Creators", embedUrl: "https://www.youtube.com/embed/5qap5aO4i9A", watchUrl: "https://www.youtube.com/watch?v=5qap5aO4i9A", desc: "Tips for discovering creators that fit your brand." },
    { videoId: "jNQXAC9IVRw", title: "Pricing & Negotiation Tips", embedUrl: "https://www.youtube.com/embed/jNQXAC9IVRw", watchUrl: "https://www.youtube.com/watch?v=jNQXAC9IVRw", desc: "Setting rates and working with budgets." },
    { videoId: "9bZkp7q19f0", title: "Content Brief Best Practices", embedUrl: "https://www.youtube.com/embed/9bZkp7q19f0", watchUrl: "https://www.youtube.com/watch?v=9bZkp7q19f0", desc: "Writing briefs that get great content." },
]);
...
const testimonials = ref([
    { quote: "Super easy to find creators that fit our brand. Saved us so much time.", name: "Demo User", role: "Brand Manager", avatar: "https://picsum.photos/seed/t1/80/80" },
    { quote: "The platform is intuitive and the creators are professional. Highly recommend.", name: "Demo Client", role: "Marketing Lead", avatar: "https://picsum.photos/seed/t2/80/80" },
    { quote: "We scaled our influencer campaigns 3x in six months. Game changer.", name: "Demo Founder", role: "Founder", avatar: "https://picsum.photos/seed/t3/80/80" },
]);

const faqs = ref([
    { question: "What is influencer marketing?", answer: "Influencer marketing is a form of marketing where brands collaborate with social media influencers and content creators to promote products or services." },
    { question: "How does StarJD work?", answer: "StarJD connects brands with vetted creators. Brands search by platform, category, and budget; message creators; and book packages. Payment is held securely until content is delivered." },
    { question: "How much does it cost?", answer: "Costs vary by creator reach and platform. Many creators offer packages starting under ₹5,000. No upfront platform fee — you pay per project." },
    { question: "Which platforms are supported?", answer: "We support Instagram, TikTok, YouTube, Twitter, and UGC creators. Filter by platform in the search bar to find the right fit." },
]);
...
function updateRing() {
    ...
    ring.value = {
        x: ring.value.x + dx * 0.15,
        y: ring.value.y + dy * 0.15,
    };
...
onMounted(() => {
    ...
    axios.get("/api/creators/options/filters").then((r) => {
