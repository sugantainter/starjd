<template>
    <div class="min-h-screen bg-[#fafaf9] text-[#1a1a1a]">
        ...
        <!-- Category pills (Replacement for Platform pills) -->
        <section class="border-b border-[#e5e7eb] bg-white px-4 py-6">
            <div
                class="mx-auto flex max-w-6xl flex-wrap items-center justify-center gap-3"
            >
                <button
                    class="cursor-link rounded-full border border-[#e5e7eb] px-5 py-2.5 text-sm font-bold transition hover:border-[#e63946] hover:bg-[#e63946]/5 hover:text-[#e63946]"
                    :class="{
                        'border-[#e63946] bg-[#e63946] text-white shadow-lg shadow-[#e63946]/20':
                            activeCategory === 'All',
                    }"
                ...
                <button
                    v-for="cat in heroFilterOptions.categories"
                    :key="cat.id"
                    class="cursor-link rounded-full border border-[#e5e7eb] px-5 py-2.5 text-sm font-bold transition hover:border-[#e63946] hover:bg-[#e63946]/5 hover:text-[#e63946]"
                    :class="{
                        'border-[#e63946] bg-[#e63946] text-white shadow-lg shadow-[#e63946]/20':
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full bg-[#f59e0b] px-2 py-0.5 text-xs font-medium text-white shadow-sm"
                                    >
                                        <svg
                                            class="h-3 w-3"
                                            fill="currentColor"
                            ...
                            <div class="p-4 md:p-5">
                                ...
                                >
                                    ...
                                    <span
                                        v-if="creator.rating"
                                        class="shrink-0 text-sm font-bold text-[#1a1a1a]"
                                        >★ {{ creator.rating }}</span
                                    >
                                </div>
                                <p
                                    v-if="creator.tagline"
                                    class="mt-1 line-clamp-1 text-sm text-[#6b7280]"
                                >
                                    {{ creator.tagline }}
                                </p>
                                <p
                                ...
                                <p
                                    ...
                                    class="mt-3 text-base font-black text-[#e63946]"
                                >
...
function fetchFeaturedCreators() {
    loadingCreators.value = true;
    const params = { featured: 1, per_page: 8 };
    if (activeCategory.value !== "All") {
        params.category = activeCategory.value;
    }
    axios
        .get("/api/creators", { params })
        .then((r) => {
            const data = r.data?.data ?? r.data ?? [];
