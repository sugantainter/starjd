<template>
  <div>
    <h1 class="text-2xl font-bold text-[#1a1a1a]">Discover Creators</h1>
    <p class="mt-1 text-[#64748b]">Browse creators and start a collaboration.</p>
    <div class="mt-4 flex flex-wrap gap-2">
      ...
      <input v-model.number="filters.min_rate" type="number" min="0" step="100" placeholder="Min price (₹)" class="w-28 rounded-xl border border-[#e2e8f0] px-3 py-2 text-sm focus:border-[#e63946] focus:outline-none focus:ring-1 focus:ring-[#e63946]" />
      ...
      <button type="button" class="cursor-link rounded-xl border border-[#e2e8f0] px-4 py-2 text-sm hover:bg-[#f1f5f9]" @click="clearFilters">Clear all</button>
    </div>
    ...
    <div v-if="!list.length && !loading" class="mt-8 rounded-xl border border-[#e2e8f0] bg-white p-8 text-center text-[#64748b]">No creators found.</div>
    <div v-if="loading" class="mt-8 text-center text-[#64748b]">Loading…</div>
  </div>
</template>
...
const filters = reactive({ category: '', gender: '', language: '', min_rate: '' });
...
async function load(page = 1) {
  ...
  try {
    ...
    const res = await axios.get('/api/creators', { params, withCredentials: true });
    const d = res.data?.data ?? res.data;
    list.value = Array.isArray(d) ? d : (d && d.data) ? d.data : [];
  } finally {
    loading.value = false;
  }
}
