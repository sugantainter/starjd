    ...
    <div>
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-[#64748b]">
                Edit headline, background images, and CTA buttons for the
                homepage hero.
            </p>
        </div>
        ...
        <form
            v-else
            class="space-y-6 rounded-xl border border-[#e2e8f0] bg-white p-6 shadow-sm"
            @submit.prevent="save"
        >
            <div>
                <label class="mb-1 block text-sm font-medium text-[#1a1a1a]"
                    >Headline</label
                >
                <input
                    v-model="form.headline"
                    type="text"
                    class="w-full max-w-xl rounded-lg border border-[#e2e8f0] px-3 py-2 text-[#1a1a1a]"
                    placeholder="Influencer Marketing Made Easy"
                />
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-[#1a1a1a]"
                    >Subheadline (optional)</label
                >
                <input
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-[#1a1a1a]"
                    >Wallpaper background images</label
                >
                <p class="mb-2 text-xs text-[#64748b]">
                    Enter one URL per line, or upload images below.
                </p>
                <textarea
                    v-model="wallpaperText"
                    rows="4"
                    class="mb-2 w-full rounded-lg border border-[#e2e8f0] px-3 py-2 font-mono text-sm text-[#1a1a1a]"
                ...
                <div class="flex items-center gap-2">
                    <input
                        ...
                        class="hidden"
                        @change="onWallpaperFileSelect"
                    />
                    <button
                        type="button"
                        class="rounded-lg border border-[#e2e8f0] bg-white px-3 py-2 text-sm font-medium text-[#1a1a1a] hover:bg-[#f1f5f9]"
                        :disabled="uploadingWallpaper"
                        @click="$refs.wallpaperFileInput?.click()"
                    >
            ...
            <div>
                <label class="mb-2 block text-sm font-medium text-[#1a1a1a]"
                    >Gallery cascade images (center of hero)</label
                >
                <div class="space-y-3">
                    <div
                        v-for="(item, i) in form.cascade_images"
                        :key="i"
                        class="flex flex-wrap items-start gap-2 rounded-lg border border-[#e2e8f0] p-3"
                    >
                        <div
                            class="h-14 w-20 shrink-0 overflow-hidden rounded bg-[#f1f5f9]"
                        >
                            <StoragePreviewImg
                                v-if="item.src"
                            ...
                            <span
                                v-else
                                class="flex h-full w-full items-center justify-center text-xs text-[#94a3b8]"
                                >No image</span
                            >
                        </div>
                        <div class="min-w-0 flex-1 space-y-1">
                            <input
                                ...
                                class="w-full rounded-lg border border-[#e2e8f0] px-3 py-2 text-sm text-[#1a1a1a]"
                            />
                            <input
                                v-model="item.link"
                                type="text"
                        ...
                        <input
                            ...
                            class="w-24 rounded-lg border border-[#e2e8f0] px-3 py-2 text-sm text-[#1a1a1a]"
                        />
                        <input
                            :ref="(el) => setCascadeFileRef(el, i)"
                            type="file"
                            accept="image/jpeg,image/png,image/webp,image/gif"
                            class="hidden"
                            @change="(e) => onCascadeFileSelect(e, i)"
                        />
                        <button
                            type="button"
                            class="rounded-lg border border-[#e63946] bg-white px-3 py-2 text-sm font-medium text-[#e63946] hover:bg-[#e63946]/5"
                            :disabled="uploadingCascadeIndex !== null"
                            @click="triggerCascadeUpload(i)"
                        >
                            {{
                                uploadingCascadeIndex === i
                                    ? "Uploading…"
                                    : "Upload"
                            }}
                        </button>
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#1a1a1a]"
                        >Button 1 (Creator) label</label
                    >
                    <input
                        v-model="form.btn_creator_label"
                        type="text"
                ...
                <div>
                    ...
                    >
                    <input
                        v-model="form.btn_creator_url"
                        type="text"
                        class="w-full rounded-lg border border-[#e2e8f0] px-3 py-2 text-[#1a1a1a]"
                        placeholder="#join-creator or /creator"
                    />
                </div>
                <div class="sm:col-span-2 lg:col-span-1" />
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#1a1a1a]"
                        >Button 2 (Brand) label</label
                    >
                    <input
                        v-model="form.btn_brand_label"
                        type="text"
                        class="w-full rounded-lg border border-[#e2e8f0] px-3 py-2 text-[#1a1a1a]"
                    />
                </div>
                <div>
                    ...
                    >
                    <input
                        v-model="form.btn_brand_url"
                        type="text"
                        class="w-full rounded-lg border border-[#e2e8f0] px-3 py-2 text-[#1a1a1a]"
                        placeholder="#join-brand or /brand"
                    />
                </div>
                <div class="sm:col-span-2 lg:col-span-1" />
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#1a1a1a]"
                        >Button 3 (Browse) label</label
                    >
                    <input
                        v-model="form.btn_browse_label"
                        type="text"
                        class="w-full rounded-lg border border-[#e2e8f0] px-3 py-2 text-[#1a1a1a]"
                    />
                </div>
                <div>
                    ...
                    >
                    <input
                        v-model="form.btn_browse_url"
                        type="text"
                        class="w-full rounded-lg border border-[#e2e8f0] px-3 py-2 text-[#1a1a1a]"
                        placeholder="#featured or /creators"
                    />
                </div>
            </div>
            ...
            <div class="flex gap-2 pt-2">
                <button
                    type="submit"
                    class="rounded-lg bg-[#e63946] px-4 py-2 text-sm font-medium text-white hover:bg-[#c1121f]"
                >
                    Save Hero
                </button>
                <button
...
async function load() {
    ...
    try {
        ...
        form.wallpaper_images =
            Array.isArray(h.wallpaper_images) && h.wallpaper_images.length
                ? h.wallpaper_images.map((i) =>
                      typeof i === "string"
                          ? { src: i }
                          : { src: i?.src || "" },
                  )
        ...
        if (Array.isArray(h.cascade_images) && h.cascade_images.length) {
            form.cascade_images = h.cascade_images.map((i) => ({
                src: i.src || "",
                alt: i.alt || "",
                link: i.link || "",
            }));
            while (form.cascade_images.length < 5)
                form.cascade_images.push({ src: "", alt: "", link: "" });
        }
...
async function save() {
    const payload = {
        ...
        wallpaper_images: form.wallpaper_images?.length
            ? form.wallpaper_images
            : null,
        cascade_images: form.cascade_images?.filter((i) => i.src)?.length
            ? form.cascade_images
                  .filter((i) => i.src)
                  .map((i) => ({
                      src: i.src,
