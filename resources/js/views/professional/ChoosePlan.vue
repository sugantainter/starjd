<template>
  <div class="mx-auto max-w-2xl py-8 px-4">
    ...
    <p class="mt-2 text-[#64748b]">Pay once to access your professional dashboard. Secure payment via PayU.</p>
    ...
    <div class="mt-6 grid gap-4 sm:grid-cols-3">
      <button
        ...
        :class="selected?.id === plan.id ? 'border-[#f59e0b] bg-[#f59e0b]/5' : 'border-[#e2e8f0] hover:border-[#f59e0b]/50'"
        @click="selected = plan"
      >
    ...
    <div v-if="availableCoupons.length" class="mt-6 border-t border-[#e2e8f0] pt-6">
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        <div v-for="c in availableCoupons" :key="c.id" 
          class="flex flex-col justify-between rounded-xl border border-dashed border-[#f59e0b]/30 bg-[#f59e0b]/5 p-4 cursor-pointer hover:bg-[#f59e0b]/10 transition"
          @click="couponCode = c.code; applyCoupon()"
        >
          <div class="flex items-center justify-between mb-2">
            <span class="font-mono font-bold text-[#f59e0b]">{{ c.code }}</span>
            <span class="text-[10px] font-bold text-white bg-[#f59e0b] px-1.5 py-0.5 rounded shadow-sm">
