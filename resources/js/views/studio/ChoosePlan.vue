<template>
  <div class="mx-auto max-w-2xl py-8 px-4">
    ...
    <p class="mt-2 text-[#64748b]">Pay once to access your studio dashboard. Secure payment via PayU.</p>
    ...
    <div class="mt-6 grid gap-4 sm:grid-cols-3">
      <button
        ...
        :class="selected?.id === plan.id ? 'border-[#3b82f6] bg-[#3b82f6]/5' : 'border-[#e2e8f0] hover:border-[#3b82f6]/50'"
        @click="selected = plan"
      >
        <div class="font-semibold text-[#1a1a1a]">{{ plan.name }}</div>
        <div class="mt-2 text-2xl font-bold text-[#3b82f6]">₹{{ displayPrice(plan) }}</div>
    ...
    <div v-if="availableCoupons.length" class="mt-6 border-t border-[#e2e8f0] pt-6">
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        <div v-for="c in availableCoupons" :key="c.id" 
          class="flex flex-col justify-between rounded-xl border border-dashed border-[#3b82f6]/30 bg-[#3b82f6]/5 p-4 cursor-pointer hover:bg-[#3b82f6]/10 transition"
          @click="couponCode = c.code; applyCoupon()"
        >
          <div class="flex items-center justify-between mb-2">
            <span class="font-mono font-bold text-[#3b82f6]">{{ c.code }}</span>
            <span class="text-[10px] font-bold text-white bg-[#3b82f6] px-1.5 py-0.5 rounded shadow-sm">
    ...
    <div class="mt-8">
      <button
        ...
        class="cursor-link rounded-xl bg-[#3b82f6] px-6 py-3 font-semibold text-white transition hover:bg-[#2563eb] disabled:opacity-50"
        @click="pay"
      >
